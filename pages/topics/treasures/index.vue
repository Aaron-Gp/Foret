                ...
                <div class=" max-w-max px-6 pt-6 pb-6 mx-auto">
                    ...
                    <div class="row">
                        <div class="md:flex-[0_0_auto] md:w-6/12">
                            <div class="text-white bg-black bg-opacity-50 p-4 border-t-4 border-solid border-t-secondary">
                                <h1
                                    class="lg:mb-8 mb-4 xl:text-[3.375rem] lg:text-[3rem] md:text-[2.25rem] sm:text-[1.625rem]">
                                    {{ store.title }}</h1>
                                <p class="font-normal leading-[1.3] text-justify mb-4">{{ store.desc }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <section class="!mx-auto xl:mb-24 lg:mb-20 md:mb-16 mb-12 w-full px-3">
                    ...
                    <div class="mb-8">
                        <div class="row g-4 g-lg-5 g-xxl-6">
                            <div class="block md:flex-[0_0_auto] md:w-6/12" v-for="(page, index) in second" :key="index">
                                <a :href="'/topics/treasures/' + index">
                                    <n-card bordered
                                        class="block transition-all hover:bg-[--hover_color] hover:text-[#ffffff] hover:[--n-title-text-color:#fff] lg:hover:translate-x-[-1rem] lg:hover:translate-y-[-1rem] lg:hover:shadow-[1rem_1rem_0_#d8d8d8] line-clamp-5"
                                        :class="{ 'lg:-mt-8': index % 2 != 0 }" :style="{ '--hover_color': page.color }">
                                        <template class="title" #header>
                                            {{ page.title }}
                                        </template>
                                        <template #cover>
                                            <img class=" object-cover max-w-full object-center w-full " :src="page.cover">
                                        </template>
                                        <template #default class="line-clamp-5">
                                            {{ page.desc }}
                                        </template>
                                    </n-card>
                                </a>
...
<script setup>
const store = useContentState().treasures
